...
void show2()
{
	std::string a[5] = { "Petrov", "Ivanov", "Sidorov", "Kyznetsov", "Pushkin"};
	Array<std::string> arr(a, 5);

	std::cout << "Index of element with value 3 : " << arr.FindElem("Ivanov") << std::endl;
	arr.Sort();
	std::cout << "Sorted array:\n" << arr << std::endl;
	arr.Insert("Rybalov", 2);
	std::cout << "Inserted 30 in position 2:\n" << arr << std::endl;
	arr.ShiftLeft(2, 1);
	std::cout << "Shifted left from index 2 by 1 position:\n" << arr << std::endl;
	arr.ShiftRight(3, 1);
